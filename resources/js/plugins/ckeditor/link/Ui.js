...
export default class LinkUI extends Plugin {
	...
	getSelectionElementAttributes() {
	  const element = this._getSelectedLinkElement();

    const attrs = {};
    element._attrs.forEach((value, key) => {
      attrs[key] = value;
    });

    const classes = [];
    element._classes.forEach(item => {
      if (!item.match('ck-')) {
        classes.push(item);
      }
    });
	...
	_createActionsView() {
      ...
      // todo:update this dirty hack
      const attrs = this.getSelectionElementAttributes();
      attrs.href = linkCommand.value;
      eventBus.$emit('rich-editor.link.open', { id: this.editor.id, attrs });
	...
	_createFormView() {
		...
		this.listenTo( formView, 'submit', () => this._save(formView));
    ...
    let value = ''; //linkCommand.value;
		if (linkCommand.value) {
		  value = linkCommand.value;
    }
		if (this.linkAttributes && this.linkAttributes.href) {
		  value = this.linkAttributes.href;
    }
		this.formView.urlInputView.inputView.element.value = value;
	}
    ...
    if ( !this._getSelectedLinkElement() ) {
      eventBus.$emit('rich-editor.link.open', { id: this.editor.id});
    }
    ...
    eventBus.$on('rich-editor.link.save', attrs => {
      this.linkAttributes = attrs;
      const formView = new LinkFormView( editor.locale, linkCommand.manualDecorators );
      this._save(formView, attrs);
    });
